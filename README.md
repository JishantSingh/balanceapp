...
- Add / edit / delete entries and customers (with double-tap confirm)
...
5. Authorize when asked, then copy the **`/exec` URL**
...
| sheet | columns |
|---|---|
| `user` | `user_id`, `name`, `created_at`, `phone` |
| `transaction` | `id`, `user_name` (holds the customer's `user_id`), `date`, `type` (`given` \| `received`), `amount`, `comment` |
