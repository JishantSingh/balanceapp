...
**Retries are idempotent (v8, additive).** The phone replays every queued write until it gets an answer, so an answer lost *after* the row was written is indistinguishable from a write that never arrived — and it must retry. Therefore: `addTxn`/`addUser` accept an optional top-level `cid` (an opaque client id, one per queued write); the first insert remembers which row it made and a replay of the same `cid` returns **that** row instead of writing a second one, for 6h (CacheService — no new scope). `deleteTxn`/`deleteUser` treat "row is not here" as success (`{deleted:id, already:true}`) — a delete that arrives twice has still done what it was asked. `updateTxn`/`updateUser` deliberately keep failing on a missing row: a lost update is a real conflict, not a duplicate. Both directions of skew are safe — an old backend ignores `cid`, and an old *frontend* simply never sends one. The one crack in "error text is not contract": the frontend forgives a pre-v8 `deleteTxn`/`deleteUser` refusal whose message starts with "Transaction not found"/"Customer not found", because the alternative is rolling a delete back and resurrecting the entry. That match is the only place messages are read, and `code` (v4 plan) supersedes it.
