...
|---|---------|----------------|--------|
| 2.1 | **Reminder cohorts + due queue** | Per-customer frequency: Off / Weekly / 15 days / Monthly (**default: Monthly** for existing customers). Home shows "🔔 N reminders due"; queue dialog with one-tap WhatsApp sends; app stamps `last_reminded`. True auto-send is impossible on free WhatsApp (one tap per message required) — and auto-messaging is the researched trust-killer anyway. | 🔨 backend drafted |
